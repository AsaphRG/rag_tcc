...
class AgentWrapper:
    ...
    def __init__(self, agent):
        self.agent = agent
        from vertexai.preview import reasoning_engines
        # We wrap the ADK agent inside AdkApp to handle the ADK runtime
    ...
    def query(self, message: str, user_id: str = "laravel_user", session_id: str = "laravel_session") -> dict:
        """
        Entry point for the :query REST endpoint.
        This is what Laravel calls when accessing the URL with :query
        """
    ...
    def stream_query(self, message: str, user_id: str = "laravel_user", session_id: str = "laravel_session"):
        """
        Entry point for the :stream_query endpoint.
        This is what the --send command uses for streaming responses.
        """
        return self.app.stream_query(
            message=message,
            user_id=user_id,
            session_id=session_id
        )
...
def main(argv=None):
    ...
    if FLAGS.create:
        ...
        print(
            "Please specify one of: --create, --deploy_laravel, --delete, --list, --create_session, --list_sessions, --get_session, or --send"
        )
